...
function forms(formMessage, formRegistration) {
  $(formMessage).submit(function (e) {
    // функция для работы с формой отправки сообщения админу (внизу сайта)
    e.preventDefault();
    $.ajax({
      type: "POST",
  ...
  $(formRegistration).submit(function (e) {
    // функция для работы с формой отправки сообщения пользователю с его регистрационными данными 
    e.preventDefault();
    $.ajax({
      type: "POST",
...
function slider(_ref) {
  ...
  var currentWidth = document.documentElement.clientWidth; // получаем текущию ширину экрана (для адаптации слайдера под мобилу)

  function deleteNotDigits(str) {
    // функция для того, чтобы в строке остались только числа и превратились в числовой тип переменной
    return +str.replace(/\D/g, ''); // replace - ищем все не числа и заменяем их на ничего, + переводим переменную в числовую
  }

  var slideWidth = window.getComputedStyle(slides[1]).width,
      slideMarginLeft = window.getComputedStyle(slides[1]).marginLeft;
  var offset = 0; // отступ влево или вправо при прокрутке слайдера
  ...
  function slideNext(numSlides) {
    // функция для перелистывания слайдера вперед. Аргумент - кол-во отображаемых одновременно элементов 
  ...
  next.addEventListener('click', function () {
    ...
    slidesField.style.transform = "translateX(-".concat(offset, "px)"); // передвигаем всю ленту слайдов влево на нужную длину
  });

  function slidePrev(numSlides) {
    // функция для перелистывания слайдера назад. Аргумент - кол-во отображаемых одновременно элементов 
    if (offset == 0) {
      // если сейчас показывается первый слайд
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass, tabsImgSelector) {
  ...
  tabsParent = document.querySelector(tabsParentSelector),
      // получаем родителя, в котором ссылки на табы
...
window.addEventListener('DOMContentLoaded', function () {
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_0__["default"])('.tabs__tab-item', '.tabs__content', '.tabs__tabs-wrapper', 'tabs__tab-item_active', '.tabs__image');
  ...
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])('.contact__form', '.promo__main-form');
  Object(_modules_modal__WEBPACK_IMPORTED_MODULE_3__["default"])('.overlay', '.modal__close');
});
